...
export default function Column({ column, tasks }) { // column: { id, title }
  const { setNodeRef } = useDroppable({ id: column.id }); // Make the column a droppable area

  return ( // Render the column with its title and tasks
    <div
    ...
    >
      ...
      <div className="flex flex-col gap-3 p-4 overflow-y-auto max-h-[75vh]">
        {tasks.length === 0 ? ( // No tasks message
          <p className="text-center text-gray-400 italic">No tasks</p>
        ) : (
          tasks.map((task) => <TaskCard key={task.id} task={task} />) // Render each task card
        )}
      </div>
    </div>
  );
