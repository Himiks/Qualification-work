...
function AdminDashboard() {
  ...
  const fetchTasks = async () => { // Fetch all tasks
    try {
      const res = await adminService.getTasks(); // API call to get tasks
      setTasks(res);
    } catch (err) {
  ...
  const handleDelete = async (id) => { // Delete task handler
    ...
    try {
      await adminService.deleteTask(id); // API call to delete task
      setTasks(tasks.filter((t) => t.id !== id)); // Update state after deletion
    } catch (err) {
  ...
  const handleEdit = (id) => { // Edit task handler
  ...
  const handleSearch = async (e) => { // Search input handler
    ...
    if (value.trim() === "") { // If input is empty, fetch all tasks
      fetchTasks();
      return;
    }

    try { // Search tasks by keyword
      const res = await adminService.searchTask(value); // API call to search tasks
      setTasks(res);
    } catch (err) {
          ...
          {tasks.map((task) => ( // Task cards
            ...
            >
              ...
              <div className="absolute top-3 right-3 flex gap-2">
                <button
                  onClick={() => navigate(`/admin/task/${task.id}/details`)} // View details handler
              ...
              <div className="text-sm text-gray-700 space-y-1 mt-auto">
                ...
                <p>
                  ...
                  {task.employeeName} {/* Assigned user */}
                </p>

                <div className="flex items-center gap-2 mt-2">
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold ${getPriorityColor(
                      task.priority /* Priority color */
                    )}`}
                  >
                    {task.priority}
                  </span>
                 ...
                 <button
                      onClick={() => {
                        if (task.technique && task.technique !== "NONE") { // Navigate to technique if assigned
                          navigate(`/techniques/${task.technique.toLowerCase()}/${task.id}`);
                        }
                      }}
                      className={`ml-2 px-3 py-1 text-xs rounded-full font-semibold shadow-md transition-all ${ // Start technique button
